...
public class Documento {
    ...
    /**
     * REPORTE, BEAMER, PDF_EXCEL o DESCONOCIDO
     */
    ...
    public <T> T accept(DocumentoVisitor<T> visitor) {
        String tipo = tipoDetectado == null ? "" : tipoDetectado.trim().toUpperCase();

        return switch (tipo) {
            case "REPORTE" -> visitor.visitarReporte(this);
            case "BEAMER" -> visitor.visitarBeamer(this);
            case "PDF_EXCEL" -> visitor.visitarPdfExcel(this);
            default -> visitor.visitarDesconocido(this);
        };
    }
}